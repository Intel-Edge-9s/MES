...
enum MFG_COIL_ADDR {
    /* Write (C -> PLC 제어/펄스) */
    MFG_COIL_START          = 24, // %QX3.0
    MFG_COIL_STOP           = 25, // %QX3.1
    MFG_COIL_MAIN_MERGE     = 31, // %QX3.7 (제조 완성)


    /* Read (PLC -> C 모니터링) */
    MFG_COIL_RUN_MAIN       = 28, // %QX3.4
    MFG_COIL_RUN_MID        = 36, // %QX4.4
    MFG_COIL_RUN_LOW        = 44, // %QX5.4

};

enum MFG_REG_ADDR {
    MFG_REG_FINAL_TOTAL = 1034  // %MW10 (INT16) 최종 생산 수량
};
...
static void modbus_poll_cb(UA_Server *server, void *data) {
    ...
    if(mb_read_bit_retry(MFG_COIL_RUN_MAIN, &b) == 0) {
        ...
        UA_Variant vv; UA_Variant_setScalar(&vv, &v, &UA_TYPES[UA_TYPES_BOOLEAN]);
        (void)UA_Server_writeValue(server, RUN_MAIN_ID, vv); /* UA 값 갱신(전달용) */
    }
...
static void production_cb(UA_Server *server, void *data) {
...

    {
        ...
        UA_StatusCode rr = UA_Server_readValue(server, FINAL_TOTAL_SET_ID, &vv);

        if(rr == UA_STATUSCODE_GOOD &&
        UA_Variant_hasScalarType(&vv, &UA_TYPES[UA_TYPES_UINT16])) {

            UA_UInt16 target = *(UA_UInt16*)vv.data;

            if(target > 0 && g_total_count >= (uint64_t)target) {
                printf("[MFG] Target reached (%lu/%u) -> AUTO STOP\n",
                    (unsigned long)g_total_count, (unsigned)target);

                mb_pulse_coil(MFG_COIL_STOP, 200);
                g_mfg_running = false;
                ...
                UA_Variant_setScalar(&vs, &stopped, &UA_TYPES[UA_TYPES_STRING]);
                (void)UA_Server_writeValue(server, STATUS_ID, vs);

                UA_Variant_clear(&vv);
                return; /* ✅ AUTO STOP 이후 즉시 종료(가장 안전) */
            }
        }
...
StartOrder_cb(UA_Server *server,
    ...
    g_mfg_running = true;
        ...
        /* ---- Read final_total_set from UA and write to OpenPLC MW10(1034) 시작할 때도 입력---- */
    {
        UA_Variant vv; UA_Variant_init(&vv);
        UA_StatusCode rr = UA_Server_readValue(server, FINAL_TOTAL_SET_ID, &vv);
        ...
        if(rr == UA_STATUSCODE_GOOD &&
           UA_Variant_hasScalarType(&vv, &UA_TYPES[UA_TYPES_UINT16])) {
            ...
            UA_UInt16 ft = *(UA_UInt16*)vv.data;
            if(mb_write_reg_u16_retry(MFG_REG_FINAL_TOTAL, (uint16_t)ft) == 0) {
                printf("[MFG][MODBUS] MW10 set: addr=%d value=%u\n",
                       MFG_REG_FINAL_TOTAL, (unsigned)ft);
            } else {
                printf("[MFG][MODBUS] MW10 set failed\n");
            }
        } else {
            printf("[MFG] final_total_set read failed (rr=0x%08x)\n", (unsigned)rr);
        }

        UA_Variant_clear(&vv);
    }

    /* ---- PLC START trigger (coil 24) ---- */
    printf("set_coil 이전에 멈춤\n");
...
/*--------------- 총생산량 ctrl-------------------------*/
{
    ...
    UA_UInt16 v0 = 0; /* 목표 수량 초기값 */
    UA_Variant_setScalar(&a.value, &v0, &UA_TYPES[UA_TYPES_UINT16]);

    /* ✅ Client가 write 해야 하므로 RW */
    ...
    if(rc2 != UA_STATUSCODE_GOOD)
        printf("[MFG] add final_total_set failed: 0x%08x\n", (unsigned)rc2);
}

    /* FINAL_TOTAL_SET_ID에 "쓰기 콜백" 연결 (Write 즉시 MW10 반영) */
    {
        UA_ValueCallback cb;
        memset(&cb, 0, sizeof(cb));
        cb.onWrite = onWrite_final_total_set;
        UA_Server_setVariableNode_valueCallback(server, FINAL_TOTAL_SET_ID, cb);
        printf("[MFG] final_total_set valueCallback attached\n");
    }


/* RUN_MAIN (Boolean, ReadOnly) */
{
    UA_VariableAttributes a = UA_VariableAttributes_default;
    ...
    if(rc2 != UA_STATUSCODE_GOOD)
        printf("[MFG] add run_main failed: 0x%08x\n", (unsigned)rc2);
}


    /* RUN_MID (Boolean, ReadOnly) */
    {
        UA_VariableAttributes a = UA_VariableAttributes_default;
        ...
        if(rc2 != UA_STATUSCODE_GOOD)
            printf("[MFG] add run_mid failed: 0x%08x\n", (unsigned)rc2);
    }

    /* RUN_LOW (Boolean, ReadOnly) */
    {
        UA_VariableAttributes a = UA_VariableAttributes_default;
        ...
        if(rc2 != UA_STATUSCODE_GOOD)
            printf("[MFG] add run_low failed: 0x%08x\n", (unsigned)rc2);
    }
