...
/*===========produce data======================*/
static UA_UInt64 g_prod_count = 0;
static UA_UInt64 g_defect_count = 0;
static UA_Double g_defect_rate = 0.0;
static UA_UInt16 g_defect_code = 0;
static UA_UInt64 g_attempt = 0;
...
static UA_Boolean load_pki_once(void) {
    ...
    g_clientCert = loadFile("/home/pi240/opcua_prac/certs/mes/cert.der");
    g_clientKey  = loadFile("/home/pi240/opcua_prac/certs/mes/key.der");
    g_trustMfg   = loadFile("/home/pi240/opcua_prac/certs/mes/trust_mfg.der");
    g_trustLog   = loadFile("/home/pi240/opcua_prac/certs/mes/trust_log.der");
...
static void print_prod_status(void)
{
    printf("[MES][PROD] count=%llu  defect=%llu  rate=%.2f%%  code=%u(%s)\n",
           (unsigned long long)g_prod_count,
           (unsigned long long)g_defect_count,
           g_defect_rate,
           g_defect_code,
                         ...
                         UA_UInt32 monId, void *monCtx, UA_DataValue *value)
{
    ...
    /* 온습도 터미널 출력 제거*/
    if(strcmp(tag, "Temp") == 0 || strcmp(tag, "Humidity") == 0 ||
        strcmp(tag, "LOG_Temp") == 0 || strcmp(tag, "LOG_Humidity") == 0) {
        return; // 출력 안 함 (UI에서만 사용)
    }

    /* ---- Production nodes: 받으면 상태 갱신 후 4개 묶어서 출력 ---- */
    if(strcmp(tag, "ProdCount")==0 && UA_Variant_hasScalarType(v,&UA_TYPES[UA_TYPES_UINT64])) {
    g_prod_count = *(UA_UInt64*)v->data; return;
    }
    if(strcmp(tag, "DefectCount")==0 && UA_Variant_hasScalarType(v,&UA_TYPES[UA_TYPES_UINT64])) {
        g_defect_count = *(UA_UInt64*)v->data; return;
    }
    if((strcmp(tag,"DefectRate(%)")==0 || strcmp(tag,"DefectRate(%%)")==0) &&
    UA_Variant_hasScalarType(v,&UA_TYPES[UA_TYPES_DOUBLE])) {
        g_defect_rate = *(UA_Double*)v->data; return;
    }
    if(strcmp(tag,"DefectCode")==0 && UA_Variant_hasScalarType(v,&UA_TYPES[UA_TYPES_UINT16])) {
        g_defect_code = *(UA_UInt16*)v->data; return;
    }

    /* ✅ attempt가 바뀔 때마다 “생산 1회”로 보고 한 번만 출력 */
    if(strcmp(tag,"Attempt")==0 && UA_Variant_hasScalarType(v,&UA_TYPES[UA_TYPES_UINT64])) {
        g_attempt = *(UA_UInt64*)v->data;
        printf("[MES][PROD] attempt=%llu ok=%llu defect=%llu rate=%.2f%% code=%u(%s)\n",
...
int main(void) {
    ...
    UA_UInt32 subLogId = respL.subscriptionId;

    UA_NodeId logTempId = UA_NODEID_STRING(1, "log/temp");
    UA_NodeId logHumId  = UA_NODEID_STRING(1, "log/hum");
    
    UA_NodeId tempId = UA_NODEID_STRING(1, "mfg/temp");
    UA_NodeId humId  = UA_NODEID_STRING(1, "mfg/hum");
    UA_NodeId prod_countId  = UA_NODEID_STRING(1, "mfg/prod_count");
    UA_NodeId defect_countId  = UA_NODEID_STRING(1, "mfg/defect_count");
    UA_NodeId defect_rateId  = UA_NODEID_STRING(1, "mfg/defect_rate");
    UA_NodeId defect_codeId  = UA_NODEID_STRING(1, "mfg/defect_code");
    UA_NodeId attemptId = UA_NODEID_STRING(1, "mfg/attempt_count");
    
    
    add_mon(log, subLogId, logTempId, "LOG_Temp");
    add_mon(log, subLogId, logHumId,  "LOG_Humidity");

    add_mon(mfg, subId, tempId, "Temp");
    add_mon(mfg, subId, humId, "Humidity");
    add_mon(mfg, subId, prod_countId, "ProdCount");
    add_mon(mfg, subId, defect_countId, "DefectCount");
    add_mon(mfg, subId, defect_rateId, "DefectRate(%%)");
    add_mon(mfg, subId, defect_codeId, "DefectCode");
    add_mon(mfg, subId, attemptId, "Attempt");
    ...
    while(g_running) {
        ...
        if(atomic_exchange(&g_log_start, 0)) {
            write_speed(log, "log/conveyor_speed1", 60.0);
            write_speed(log, "log/conveyor_speed2", 50.0);
            write_speed(log, "log/conveyor_speed3", 40.0);
